...
import type { IPreprocessor, PreprocessArgs, PreprocessResults } from './index';
...
export class GaussianPreprocessor implements IPreprocessor {
  private bindings = new WeakMap<PointCloud, {
    output: GPUBuffer; gaussian: GPUBuffer; sh: GPUBuffer;
    pc: GPUBindGroup; settings: GPUBindGroup;
  }>();
  dispose(): void {
    this.cameraUniforms?.destroy();
    this.settingsUniforms?.destroy();
  ...
  dispatchModel(args: {
    ...
    let cached = this.bindings.get(args.pointCloud);
    if (!cached || cached.output !== args.global.splat2D || cached.gaussian !== pcBuf.gaussianBuffer || cached.sh !== pcBuf.shBuffer) {
    ...
    cached = { output: args.global.splat2D, gaussian: pcBuf.gaussianBuffer,
      sh: pcBuf.shBuffer, pc: pcBG, settings: smBG };
    this.bindings.set(args.pointCloud, cached);
    }
    computePass.setBindGroup(1, cached.pc);
    computePass.setBindGroup(2, this.getSortBindGroup(args.sortStuff));
    computePass.setBindGroup(3, cached.settings);
